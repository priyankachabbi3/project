class EventsController < ApplicationController
  ...
  # POST /events.json
  def create
    date = params[:event][:start].split("/")  unless params[:event][:start].blank?
    params[:event][:start] = date.insert(1,date.delete_at(0)).join("-").to_date.strftime("%Y-%m-%d") unless params[:event][:start].blank?
